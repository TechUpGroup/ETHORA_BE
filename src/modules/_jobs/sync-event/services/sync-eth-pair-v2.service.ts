...
import { Erc20__factory, FactoryV2__factory } from "common/abis/types";
import { Types } from "mongoose";
import { PairCreatedEvent } from "common/abis/types/FactoryV2";
import { ContractName } from "common/constants/contract";

@Injectable()
export class JobSyncEthPairV2Service {
  ...
  private async start() {
    try {
      const factorys = await this.contractService.getContractByNames(ContractName.FACTORY_V2);
        ...
        try {
          try {
            await this.helperService.excuteSync({
              contract: factory,
              network,
              acceptEvents: ["PairCreated"],
              ABI: FactoryV2__factory.abi,
              callback: this.handleEvents,
            });
          } catch (err) {
            console.log("JobSyncServiceV2 -> start: ", err);
          }
        } finally {
      ...
      console.log("JobSyncServiceV2 -> querydb: ", err);
    }
  }
  ...
  private handleEvents = async ({ events: listEvents, contract, eventHashes }: IEventParams) => {
    try {
      ...
      for (const event of events) {
        const { token0, token1, pair } = (event as PairCreatedEvent).args;
        if (this.lisStableCoin.includes(token0.toLowerCase()) || this.lisStableCoin.includes(token1.toLowerCase())) {
          ...
          const indexPair = ethCreateArr.findIndex((ethPair) => ethPair.pair.toLowerCase() === pair.toLowerCase());
          if (indexPair === -1) {
      ...
      console.log("JobSyncServiceV2 -> handleEvents: ", err);
    }
  };
}
